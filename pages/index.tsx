...
const STEPS = [
  { id: 'parse', label: 'Parse product' },
  { id: 'generate', label: 'Generate SEO pack' },
...
export default function Home() {
  ...
  for (const f of (PRODUCT as any).inputs || []) initialInputs[f.key] = ''
  ...
  const [ruleResults, setRuleResults] = useState<Array<{ ruleId: string; name: string; passed: boolean; message: string; category: string }>>([])
  ...
  const inputFields: Array<{ key: string; label: string; type?: string; placeholder?: string; options?: string[] }> = (PRODUCT as any).inputs || []
  ...
  async function loadRuns() {
    try {
      ...
      const data = await r.json()
      if (r.ok && data.ok) setRuns(data.runs || [])
    } catch {
      /* optional */
    }
  }
  ...
  async function runStudio(e?: FormEvent) {
    ...
    try {
      for (let i = 0; i < STEP_IDS.length; i++) {
        ...
        rid = data.runId
        collected.push(String(data.result || ''))
        setResult(collected.join('\n\n— — —\n\n'))
        if (data.demo) setDemo(true)
        if (data.rulesetVersion) setRulesetVersion(String(data.rulesetVersion))
        if (Array.isArray(data.ruleResults)) setRuleResults(data.ruleResults)
        setRunId(rid)
      }
      showToast('SEO pack complete')
      await loadRuns()
    } catch (e: any) {
  ...
  async function exportRuns() {
    try {
      const r = await fetch('/api/runs?format=json')
      if (!r.ok) {
      ...
      a.href = url
      a.download = 'storeseo-runs.json'
      a.click()
      URL.revokeObjectURL(url)
      showToast('Exported run history')
    } catch {
      showToast('Export failed')
    }
  }
  ...
  async function sendFeedback(type: 'thumbs_up' | 'thumbs_down') {
    if (!runId) {
      showToast('Run the workflow first')
      return
    }
    try {
  ...
  return (
    <>
      <Head>
        ...
        <link rel="canonical" href="https://storeseo.lxsaihub.com/" />
      </Head>
      ...
      <div className="min-h-screen bg-white text-slate-900">
        ...
        <section className="bg-slate-950 text-white py-10">
          <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
            {[['8 rules', 'Listing checks'], ['3-step', 'SEO workflow'], ['5', 'Alt texts / page'], ['<60s', 'Per page']].map(([n, l]) => (
              <div key={l}>
        ...
        <section id="features" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold mb-3">Built for store owners, not SEO agencies</h2>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
              {featCards.map((f) => (
                <div key={f} className="rounded-2xl border border-slate-200 p-6 shadow-sm hover:-translate-y-1 transition bg-white">
                  ...
                  <p className="text-sm text-slate-600">Optimize every product page without hiring an SEO consultant.</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="how" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-extrabold">A real listing-SEO workflow in 3 steps</h2>
            </div>
            <div className="grid md:grid-cols-3 gap-6">
              {STEPS.map((s, i) => (
                <div key={s.id} className="bg-white rounded-2xl border border-slate-200 p-6">
                  ...
                  <p className="text-slate-600 text-sm">Each stage is saved as a run you can re-open and export.</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="studio" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            ...
            {demo ? (
              <div className="max-w-3xl mx-auto mb-6 rounded-2xl border border-amber-300 bg-amber-50 px-5 py-3 text-sm font-semibold text-amber-900">
                Demo mode — not live AI. The output above is a sample, not a real SEO pack.
              </div>
            ) : null}
            {error ? (
              <div className="max-w-3xl mx-auto mb-6 rounded-2xl border border-rose-300 bg-rose-50 px-5 py-3 text-sm font-semibold text-rose-700">
                {error}
              </div>
            ) : null}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <form onSubmit={runStudio} className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                  ...
                  <div key={f.key} className="mb-4">
                    ...
                    {f.type === 'select' ? (
                      <select
                        ...
                        value={formInputs[f.key] || ''}
                        onChange={(e) => setFormInputs((s) => ({ ...s, [f.key]: e.target.value }))}
                      >
                        {(f.options || []).map((o) => (
                ...
                <button type="submit" disabled={toolBusy} className="w-full py-3 rounded-full bg-indigo-600 text-white font-bold disabled:opacity-60">
                  {toolBusy ? 'Running workflow…' : (PRODUCT as any).ctaLabel || 'Run'}
                </button>

                <div className="mt-5">
                  <div className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Workflow progress</div>
                  <div className="space-y-2">
                    {STEPS.map((s, i) => {
                      const state = currentStep === i ? 'active' : currentStep > i ? 'done' : 'todo'
                      return (
                        <div key={s.id} className="flex items-center gap-3 text-sm">
                          <span className={`w-5 h-5 rounded-full grid place-items-center text-xs font-bold ${state === 'done' ? 'bg-emerald-500 text-white' : state === 'active' ? 'bg-indigo-600 text-white animate-pulse' : 'bg-slate-200 text-slate-500'}`}>
                            {state === 'done' ? '✓' : i + 1}
                          </span>
                          <span className={state === 'todo' ? 'text-slate-400' : 'text-slate-800 font-semibold'}>{s.label}</span>
                        </div>
                      )
                    })}
                  </div>
              ...
              <div className="rounded-2xl border border-slate-200 p-6 bg-white shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {rulesetVersion ? (
                    <span className="text-xs font-semibold bg-slate-100 px-2 py-1 rounded-full">ruleset v{rulesetVersion}</span>
                  ) : null}
                </div>
                <pre className="whitespace-pre-wrap text-sm bg-slate-50 border border-slate-200 rounded-xl p-4 min-h-[180px] max-h-[420px] overflow-auto">{result || 'Your SEO pack will appear here.'}</pre>

                {ruleResults.length ? (
                  <div className="mt-4">
                    <div className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Rule checks</div>
                    <ul className="space-y-1 text-sm">
                      {ruleResults.map((rr) => (
                        <li key={rr.ruleId} className="flex items-start gap-2">
                          <span className={rr.passed ? 'text-emerald-600 font-bold' : 'text-rose-600 font-bold'}>{rr.passed ? 'PASS' : 'WARN'}</span>
                          <span className="text-slate-600">{rr.message}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <div className="flex flex-wrap gap-3 mt-4">
                  ...
                  <button type="button" className="px-4 py-2 rounded-full border border-slate-200 font-semibold" onClick={exportRuns}>
                    Export runs (JSON)
                  </button>
                  {runId ? (
                    <>
                      <button type="button" className="px-4 py-2 rounded-full border border-slate-200 font-semibold" onClick={() => sendFeedback('thumbs_up')}>👍</button>
                      <button type="button" className="px-4 py-2 rounded-full border border-slate-200 font-semibold" onClick={() => sendFeedback('thumbs_down')}>👎</button>
                    </>
                  ) : null}
                ...
                {runs.length ? (
                  <div className="mt-5 text-xs text-slate-500">
                    {runs.length} previous run(s) stored on this device. Use “Export runs (JSON)” to download.
                  </div>
                ) : null}
              </div>
            </div>
        ...
        <section id="testimonials" className="py-20 bg-slate-50">
          <div className="max-w-6xl mx-auto px-6">
            <h2 className="text-3xl md:text-4xl font-extrabold text-center mb-12">Loved by store operators</h2>
            <div className="grid md:grid-cols-3 gap-5">
              {[
                ['[A]', '[First name] [Last initial].', '[Role]', '&quot;[Verbatim quote, with permission]&quot;'],
                ['[B]', '[First name] [Last initial].', '[Role]', '&quot;[Verbatim quote, with permission]&quot;'],
                ['[C]', '[First name] [Last initial].', '[Role]', '&quot;[Verbatim quote, with permission]&quot;'],
              ].map(([av, nm, role, q]) => (
            ...
            <p className="text-center text-xs text-slate-400 mt-6">
              Template placeholders — replace with real, permission-granted testimonials. We never publish fabricated customer names or quotes.
            </p>
          </div>
        </section>
        ...
        <section id="pricing" className="py-20">
          <div className="max-w-6xl mx-auto px-6">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {pricing.map((tier, i) => (
                <div key={tier.tier} className={`rounded-2xl border p-6 ${i === 1 ? 'border-2 border-indigo-600 relative shadow-lg' : 'border border-slate-200'}`}>
                  {i === 1 ? (
                  ...
                  <div className="text-4xl font-black my-3">{tier.price}</div>
                  <ul className="space-y-2 text-sm text-slate-600 mb-6">
                    {tier.desc.split(' · ').map((d: string, k: number) => (
                      <li key={k} className="flex gap-2"><Check /> {d}</li>
                    ))}
                  </ul>
                  {i === 1 ? (
          ...
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="space-y-3">
              {[
                ...
                [
                  'What happens if I hit the Fair Use limit?',
                  'Pro includes about 300 product pages/month (and a daily cap) on gpt-4o-mini. If you hit the limit, Studio returns an upgrade prompt (HTTP 429) instead of a fake result — we never show a mock as if it were a real pack.',
                ],
                ['What are the listing rules?', 'Eight named e-commerce listing rules (title length, description length, keyword guardrails, five alt texts, schema.org/Product blurb, price/availability, unique title, rich-result readiness) tag every pack so the output is reproducible.'],
        ...
        <section className="py-20">
          <div id="signup" className="max-w-3xl mx-auto px-6 text-center rounded-3xl bg-slate-950 text-white p-10">
            ...
            <p className="text-slate-300 mb-6">Join store owners using {PRODUCT.name}. Free to try — no card needed.</p>
